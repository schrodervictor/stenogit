...
`config-tracker-watch` is a long-running process running an
...
1. **Subprocess** — exec `config-tracker-commit` directly, with the
...
2. **Source the commit script and call its `main`** — slightly faster,
   ...
   loop in isolation.
3. **`systemctl --user start config-tracker@<name>.service`** — most
...
Option 1 — subprocess. The watch script invokes a `CONFIG_TRACKER_COMMIT`
command (defaults to `config-tracker-commit` on `PATH`), so tests can
