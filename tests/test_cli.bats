...
@test "add writes the conf file with expected keys" {
    ...
    local conf="$CONFIG_TRACKER_CONFIG_DIR/myinst.conf"
    [ -f "$conf" ]
...
@test "add --schedule writes a timer drop-in with the interval" {
    ...
    local dropin="$CONFIG_TRACKER_SYSTEMD_DIR/config-tracker@myinst.timer.d/schedule.conf"
    [ -f "$dropin" ]
...
@test "add --watch enables the watch service" {
    ...
    grep -q "enable --now config-tracker-watch@myinst.service" "$SYSTEMCTL_LOG"
}
...
@test "add (default) enables the timer" {
    ...
    mkdir -p "$dir"
    cmd_add "myinst" "$dir"
    grep -q "enable --now config-tracker@myinst.timer" "$SYSTEMCTL_LOG"
}
...
@test "remove deletes the conf file" {
    ...
    mkdir -p "$dir"
    cmd_add "myinst" "$dir"
    [ -f "$CONFIG_TRACKER_CONFIG_DIR/myinst.conf" ]
    cmd_remove "myinst"
    [ ! -f "$CONFIG_TRACKER_CONFIG_DIR/myinst.conf" ]
}
...
@test "remove disables the systemd units" {
    ...
    : > "$SYSTEMCTL_LOG"
    cmd_remove "myinst"
    grep -q "disable --now config-tracker@myinst.timer" "$SYSTEMCTL_LOG"
    grep -q "disable --now config-tracker-watch@myinst.service" "$SYSTEMCTL_LOG"
}
...
@test "remove deletes the timer drop-in" {
    ...
    local dropdir="$CONFIG_TRACKER_SYSTEMD_DIR/config-tracker@myinst.timer.d"
    [ -d "$dropdir" ]
    cmd_remove "myinst"
    [ ! -d "$dropdir" ]
}
